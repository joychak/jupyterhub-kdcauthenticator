...
def create_ccname(spawner):
    import os, pwd, shutil

    x = pwd.getpwnam(spawner.user.name)
    ccname = '/tmp/spark-magic-%s' % x.pw_uid
    fd = os.open(ccname, os.O_WRONLY | os.O_CREAT, 0o600)
    os.chown(fd, x.pw_uid, x.pw_gid)
    os.close(fd)

    cfg = '/home/%s/.sparkmagic/config.json' % spawner.user.name
    os.makedirs(os.path.dirname(cfg), exist_ok=True)
    os.chown(os.path.dirname(cfg), x.pw_uid, x.pw_gid)
        ...
        f.write("""
{
    ...
    "kernel_scala_credentials" : {
        "username": "",
        "password": "",
        "url": "http://bdiplivyprod.bdns.bloomberg.com:8998"
    },
    ...
    "wait_for_idle_timeout_seconds": 600,
    "livy_session_startup_timeout_seconds": 600,
        "livy_principal": "bdipsparkmagicusr@ADPRD.BLOOMBERG.COM",
    "support_proxyUser": true
}
        """)

    os.chown(cfg, x.pw_uid, x.pw_gid)

    ## Demo notebook
    demo = '/home/%s/GettingStarted.ipynb' % spawner.user.name
    src = '/etc/jupyter/GettingStarted.ipynb'
    #shutil.copyfile(src, demo)
    #os.chown(demo, x.pw_uid, x.pw_gid)

    return ccname

#c.KDCAuthenticator.service_name = 'HTTP@172.17.0.4'
#c.KDCAuthenticator.krb5_conf = '/bb/rda/notebook/keytabs/krb5.conf'
#c.KDCAuthenticator.service_name = 'HTTP@bdipjhubprod.bdns.bloomberg.com'

c.KerberosSpawner.livy_keytab_file = '/bb/rda/notebook/keytabs/bdipsparkmagicusr.keytab'
c.KerberosSpawner.livy_principal = 'bdipsparkmagicusr@ADPRD.BLOOMBERG.COM'
c.KerberosSpawner.encryption_key_location = '/bb/rda/notebook/encryption/keys'
c.KerberosSpawner.environment = {
    'KRB5CCNAME' : create_ccname,
    'KRB5_CONFIG' : '/bb/rda/notebook/keytabs/krb5.conf'
}

c.Spawner.notebook_dir = '/bb/rda/notebook/jupyter-notebooks'
c.JupyterHub.spawner_class = 'kdcspawner.kdcspawner.KerberosSpawner'

# The PAM service to use for authentication.
# c.PAMAuthenticator.service = 'login'
#c.Authenticator.whitelist = {'testuser1', 'testuser2'}
#c.Authenticator.admin_users = {'testuser1'}
